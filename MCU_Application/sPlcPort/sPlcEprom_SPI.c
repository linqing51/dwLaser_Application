...
__forceinline HAL_StatusTypeDef epromReadByte(uint32_t addr, uint8_t *rdat){//读1字节 addr: 24位地址（MB85RS2MTA 只用低18位） rdat: 输出1字节
	...
	__ASM volatile ("NOP");
  tempBuf[0] = READ;
  ret = HAL_SPI_Transmit(&CONFIG_EPROM_BUS_HANDLE, tempBuf, 1, CONFIG_EPROM_TIMEOUT);
  if(CONFIG_EPROM_SIZE > 0xFFFF){
		tempBuf[0] = (addr >> 16) & 0xFF;
		tempBuf[1] = (addr >> 8)  & 0xFF;
		tempBuf[2] =  addr        & 0xFF;
    ret = HAL_SPI_Transmit(&CONFIG_EPROM_BUS_HANDLE, tempBuf, 3, CONFIG_EPROM_TIMEOUT);
	}
	else{
    tempBuf[0] = (addr >> 8)  & 0xFF;
    tempBuf[1] =  addr        & 0xFF;
    ret = HAL_SPI_Transmit(&CONFIG_EPROM_BUS_HANDLE, tempBuf, 2, CONFIG_EPROM_TIMEOUT);
	}
...
__forceinline HAL_StatusTypeDef epromRead(uint32_t addr, uint8_t *pBuf, uint16_t len){//批量读addr: 起始地址pBuf: 输出缓冲区len: 字节数
  ...
  if(CONFIG_EPROM_SIZE > 0xFFFF){
	...
	ret = HAL_SPI_Receive(&CONFIG_EPROM_BUS_HANDLE, pBuf, len, CONFIG_EPROM_TIMEOUT);	
	EPROM_SPI_NSS_DESEL;
  __ASM volatile ("NOP");
	return ret;
}
