		...
		if(freeProtocol.cmdBufRx[0] == '$'){//扩展新指令
			...
			switch(temp16){
				case CMD_GET_LASER_TEMPERATURE:{//获取激光器温度 CMD=01
					memset(freeProtocol.cmdBufTx, 0x00, FP_TX_SIZE);
					sprintf((char*)freeProtocol.cmdBufTx, "$01%05d#", NVRAM0[EM_LASER_TEMP]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_LASER_CH1_LD:{//获取激光器通道1电流 LD1 CMD=02
					sprintf((char*)freeProtocol.cmdBufTx, "$02%05d#", NVRAM0[EM_LASER_CURRENT_CH0]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_LASER_CH2_LD:{//获取激光器通道2电流 LD2 CMD=03
					sprintf((char*)freeProtocol.cmdBufTx, "$03%05d#", NVRAM0[EM_LASER_CURRENT_CH1]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_LASER_CH1_PD:{//获取激光器通道1功率 PD1 CMD=04
					sprintf((char*)freeProtocol.cmdBufTx, "$04%05d#", NVRAM0[EM_LASER_PD_CH0]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_LASER_CH2_PD:{//获取激光器通道2功率 PD2 CMD=05
					sprintf((char*)freeProtocol.cmdBufTx, "$05%05d#", NVRAM0[EM_LASER_PD_CH0]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_TEC_CH1_CURRENT:{//获取制冷通道1电流 CMD=06
					sprintf((char*)freeProtocol.cmdBufTx, "$06%05d#", NVRAM0[EM_LASER_CURRENT_CH2]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_TEC_CH2_CURRENT:{//获取制冷通道2电流 CMD=07
					sprintf((char*)freeProtocol.cmdBufTx, "$07%05d#", NVRAM0[EM_LASER_CURRENT_CH3]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}					
				case CMD_GET_LASER_CH1_POWESET:{//获取激光器通道1功率设置 CMD=08
					sprintf((char*)freeProtocol.cmdBufTx, "$08%05d#", NVRAM0[EM_LASER_POWER_CH0]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_LASER_CH2_POWESET:{//获取激光器通道2功率设置 CMD=09
					sprintf((char*)freeProtocol.cmdBufTx, "$09%05d#", NVRAM0[EM_LASER_POWER_CH1]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_LASER_ONTIME:{//获取激光发射时间 CMD=10
					sprintf((char*)freeProtocol.cmdBufTx, "$10%05d#", NVRAM0[EM_LASER_POSWIDTH]);
					freeProtocol.txIndex = 9;
					startSendCmd();
					break;
				}
				case CMD_GET_LASER_STATUS:{//获取激光发射状态 CMD=11
					...
					//通道2激光状态
					if(LD(Y_LASER2_LED)){
						freeProtocol.cmdBufTx[4] = '1';
					}
					else{
						freeProtocol.cmdBufTx[4] = '0';
					}
					//通道1激光状态
					if(LD(Y_LASER1_LED)){
