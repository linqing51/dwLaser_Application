...
void sPlcAdcProcessBufferA(void){//循环采集ADC
	uint8_t i, j;
	uint32_t sum[CONFIG_SPLC_ADC1_CHANNEL];
	...
	if(adcBufferAselect == 0){	
		...
		adcBufferAselect = 1;
		memset((uint8_t*)sum, 0x0, (CONFIG_SPLC_ADC1_CHANNEL * 4));
		for(i = 0;i < CONFIG_SPLC_ADC1_CHANNEL;i ++){
			for(j = 0;j < CONFIG_SPLC_ADC_AVERAGE_NUM ;j ++){
				sum[i] += adcDmaBufferA0[(j * CONFIG_SPLC_ADC1_CHANNEL + i)];
		...
		NVRAM0[SPREG_ADC_45] = (uint16_t)((float)sum[0] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN6 VIN VOLTAGE
		NVRAM0[SPREG_ADC_40] = (uint16_t)((float)sum[1] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN7 LASER_NTC
		NVRAM0[SPREG_ADC_32] = (uint16_t)((float)sum[2] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN8 HT1_NTC
		NVRAM0[SPREG_ADC_33] = (uint16_t)((float)sum[3] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN9 HT2_NTC
		NVRAM0[SPREG_ADC_44] = (uint16_t)((float)sum[4] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN10 PWR_CUR		
		NVRAM0[SPREG_ADC_1] = (uint16_t)((float)sum[5] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN11 CH1_CUR
		NVRAM0[SPREG_ADC_0] = (uint16_t)((float)sum[6] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN12 CH0_CUR		
		NVRAM0[SPREG_ADC_16] = (uint16_t)((float)sum[7] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN13 TEC_CUR
		NVRAM0[SPREG_ADC_48] = (uint16_t)((float)sum[8] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN14 LPD		
		NVRAM0[SPREG_ADC_47] = (uint16_t)((float)sum[9] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN15 FPD	
		NVRAM0[SPREG_ADC_50] = (uint16_t)((float)sum[10] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//CHIP TEMP
		NVRAM0[SPREG_ADC_51] = (uint16_t)((float)sum[11] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//VREFINT
		NVRAM0[SPREG_ADC_52] = (uint16_t)((float)sum[12] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//VBAT
#endif	
		...
		NVRAM0[SPREG_ADC_9] = (uint16_t)((float)sum[3] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//FPD
		NVRAM0[SPREG_ADC_13] = (uint16_t)((float)sum[4] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//TMCU
		NVRAM0[SPREG_ADC_14] = (uint16_t)((float)sum[5] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//VREFINT
		NVRAM0[SPREG_ADC_15] = (uint16_t)((float)sum[6] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//VBAT
#endif

#if defined(MODEL_PVGLS_10W_1940_A1)
		NVRAM0[SPREG_ADC_8] = (uint16_t)((float)sum[0] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN0 MBAT NTC
		NVRAM0[SPREG_ADC_7] = (uint16_t)((float)sum[1] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN1 CH4 CUR ->TEC
		NVRAM0[SPREG_ADC_0] = (uint16_t)((float)sum[2] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN2 CH0 CUR ->1940
		NVRAM0[SPREG_ADC_1] = (uint16_t)((float)sum[3] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN3 CH1 CUR ->980
		NVRAM0[SPREG_ADC_9] = (uint16_t)((float)sum[4] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN11 FPD
		NVRAM0[SPREG_ADC_11] = (uint16_t)((float)sum[5] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN12 LASER NTC
		...
		for(i = 0;i < CONFIG_SPLC_ADC1_CHANNEL;i ++){
			for(j = 0;j < CONFIG_SPLC_ADC_AVERAGE_NUM ;j ++){
				sum[i] += adcDmaBufferA1[(j * CONFIG_SPLC_ADC1_CHANNEL + i)];
...
void sPlcAdcProcessBufferB(void){//循环采集ADC
	uint8_t i, j;
	uint32_t sum[CONFIG_SPLC_ADC3_CHANNEL];
	...
	if(adcBufferBselect == 0){	
		...
		adcBufferBselect = 1;
		memset((uint8_t*)sum, 0x0, (CONFIG_SPLC_ADC3_CHANNEL * 4));
		for(i = 0;i < CONFIG_SPLC_ADC3_CHANNEL;i ++){
			for(j = 0;j < CONFIG_SPLC_ADC_AVERAGE_NUM ;j ++){
				sum[i] += adcDmaBufferB0[(j * CONFIG_SPLC_ADC3_CHANNEL + i)];
		...
		NVRAM0[SPREG_ADC_4] = (uint16_t)((float)sum[4] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN10 PWR_CUR		
		NVRAM0[SPREG_ADC_1] = (uint16_t)((float)sum[5] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN11 CH1_CUR
		NVRAM0[SPREG_ADC_0] = (uint16_t)((float)sum[6] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN12 CH0_CUR		
		NVRAM0[SPREG_ADC_7] = (uint16_t)((float)sum[7] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN13 TEC_CUR
		NVRAM0[SPREG_ADC_10] = (uint16_t)((float)sum[8] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN14 LPD		
		NVRAM0[SPREG_ADC_9] = (uint16_t)((float)sum[9] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//IN15 FPD	
		NVRAM0[SPREG_ADC_13] = (uint16_t)((float)sum[10] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//CHIP TEMP
		NVRAM0[SPREG_ADC_14] = (uint16_t)((float)sum[11] / (float)CONFIG_SPLC_ADC_AVERAGE_NUM);//VREFINT
		...
		for(i = 0;i < CONFIG_SPLC_ADC3_CHANNEL;i ++){
			for(j = 0;j < CONFIG_SPLC_ADC_AVERAGE_NUM ;j ++){
				sum[i] += adcDmaBufferB1[(j * CONFIG_SPLC_ADC3_CHANNEL + i)];
