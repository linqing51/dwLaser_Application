...
void sPlcAdcProcessBufferA(void){//循环采集ADC
	...
	if(adcBufferAselect == 0){	
		...
		NVRAM0[SPREG_ADC_55] = (uint16_t)adc1FilterBuf[0];	//IN0 LAS0_LPD
		NVRAM0[SPREG_ADC_54] = (uint16_t)adc1FilterBuf[1];	//IN1 LAS0_FPD
		NVRAM0[SPREG_ADC_32] = (uint16_t)adc1FilterBuf[2];	//IN2 HT0_NTC
		NVRAM0[SPREG_ADC_33] = (uint16_t)adc1FilterBuf[3];	//IN3 HT1_NTC	
		NVRAM0[SPREG_ADC_52] = (uint16_t)adc1FilterBuf[4];	//IN7 IVIN_IS
		NVRAM0[SPREG_ADC_1] = (uint16_t)adc1FilterBuf[5];	//IN10 LAS1_CUR
		NVRAM0[SPREG_ADC_0] = (uint16_t)adc1FilterBuf[6];	//IN11 LAS0_CUR
		NVRAM0[SPREG_ADC_16] = (uint16_t)adc1FilterBuf[7];	//IN12 TEC0_CUR	
		NVRAM0[SPREG_ADC_40] = (uint16_t)adc1FilterBuf[8];	//IN13 LAS0_NTC
		NVRAM0[SPREG_ADC_58] = (uint16_t)adc1FilterBuf[9];//CHIP TEMP
		NVRAM0[SPREG_ADC_59] = (uint16_t)adc1FilterBuf[10];//VREFINT
		NVRAM0[SPREG_ADC_60] = (uint16_t)adc1FilterBuf[11];//VBAT		
#endif    
