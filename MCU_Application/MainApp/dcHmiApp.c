...
void updateDebugInfo(void){//更新Standby调试信息
	...
	NVRAM0[EM_LASER_DIODE_TEMP], NVRAM0[EM_HT0_TEMP], NVRAM0[EM_MCU_TEMP], NVRAM0[SPREG_ADC_9], NVRAM0[SPREG_ADC_10], NVRAM0[SPREG_DAC_7]);
	...
	NVRAM0[EM_LASER_A_DIODE_TEMP], NVRAM0[EM_WATER_HOT_TEMP], NVRAM0[EM_WATER_COOL_TEMP], NVRAM0[EM_AMBIENT0_TEMP], NVRAM0[EM_MCU_TEMP], \
...
static void temperatureLoop(void){//温度轮询轮询
	...
	//CODE转换为NTC测量温度温度
	TNTUC(EM_LASER_DIODE_TEMP, SPREG_ADC_40);
	TNTUC(EM_HT0_TEMP, SPREG_ADC_32);
	TENV(EM_MCU_TEMP, SPREG_ADC_49);
#endif
	...
	TNTLC(EM_LASER_DIODE_TEMP, SPREG_ADC_40);
	TNTLC(EM_HT0_TEMP, SPREG_ADC_32);
	TNTLC(EM_MBAT_TEMP, SPREG_ADC_46);
	TENV(EM_MCU_TEMP, SPREG_ADC_49);//CODE转换为MCU温度
#endif	
	
#if defined(LYPE_MCU_1V0_20260106)
	TNTLC(EM_LASER_A_DIODE_TEMP, SPREG_ADC_40);//激光器芯片温度
	TNTLC(EM_WATER_HOT_TEMP, SPREG_ADC_36);//水冷热端温度
	TNTLC(EM_WATER_COOL_TEMP, SPREG_ADC_37);//水冷冷端温度
	TNTLC(EM_AMBIENT0_TEMP, SPREG_ADC_56);//模拟环境温度
	TENV(EM_MCU_TEMP, SPREG_ADC_58);//CODE转换为MCU温度
#endif
	//判断二极管温度
	if(NVRAM0[EM_LASER_A_DIODE_TEMP] >= CONFIG_DIODE_HIGH_TEMP){//激光器过热
		SSET(R_LASER_TEMP_HIGH);
	}
	if(NVRAM0[EM_LASER_A_DIODE_TEMP] <= (CONFIG_DIODE_HIGH_TEMP - 50)){//激光器恢复正常温度
		RRES(R_LASER_TEMP_HIGH);
	}
	if(NVRAM0[EM_LASER_A_DIODE_TEMP] <= CONFIG_DIODE_LOW_TEMP){//激光器低温保护
		SSET(R_LASER_TEMP_LOW);
	}
	if(NVRAM0[EM_LASER_A_DIODE_TEMP] >= CONFIG_DIODE_LOW_TEMP + 50){//激光器恢复正常温度
		RRES(R_LASER_TEMP_LOW);
	}
	//判断环境温度
