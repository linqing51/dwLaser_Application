...
/*****************************************************************************/
//音频播放
void SetMusicVolume(uint8_t volume){//设置音频音量
...
void PlayMusicId(uint16_t id, uint8_t loop){//播放音频ID
	//ID 音频 loop 循环播放次数
	if(loop >= 3){
		loop = 3;
	}
...
void SuspendMusicId(void){//暂停播放
	//EE 90 02 FF FC FF FF
	BEGIN_CMD();
	TX_8(0x02);
	END_CMD();
}
void ResumeMusicId(void){//恢复播放
	//EE 90 03 FF FC FF FF 
	BEGIN_CMD();
	TX_8(0x03);
	END_CMD();
}
void StopMusicId(void){//停止播放
...
void resetGddcHmi(void){//重启GDDC HMI
    BEGIN_CMD();//EE
    TX_8(0x07);
    TX_8(0x35);
	TX_8(0x5A);
