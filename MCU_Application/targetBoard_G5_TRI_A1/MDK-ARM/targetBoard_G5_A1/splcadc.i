...
volatile uint16_t adcDmaBuffer0[(6 * 8)];
volatile uint16_t adcDmaBuffer1[(6 * 8)];
volatile uint8_t adcBufferSelect;
volatile uint8_t adcBufferDone;
 
void sPlcAdcInit(void){
	adcBufferSelect = 0;
	memset((uint8_t*)adcDmaBuffer0, 0x0, ((6 * 8) * 2));
	memset((uint8_t*)adcDmaBuffer1, 0x0, ((6 * 8) * 2));
	HAL_ADC_Start_DMA(&hadc1, (uint32_t*)&adcDmaBuffer0, (6 * 8)); 
...
void sPlcAdcProcess(void){
	...
	if(adcBufferSelect == 0){	
		...
		HAL_ADC_Start_DMA(&hadc1, (uint32_t*)&adcDmaBuffer1, (6 * 8));
		adcBufferSelect = 1;
		memset((uint8_t*)sum, 0x0, (6 * 4));
		for(i = 0;i < 6;i ++){
			for(j = 0;j < 8 ;j ++){
				sum[i] += adcDmaBuffer0[(j * 6 + i)];
			
			}
		}			

		NVRAM0[(1320 + 11)] = (uint16_t)((float)sum[0] / (float)8);
		NVRAM0[(1320 + 12)] = (uint16_t)((float)sum[1] / (float)8);
		NVRAM0[(1320 + 13)] = (uint16_t)((float)sum[2] / (float)8);
		NVRAM0[(1320 + 14)] = (uint16_t)((float)sum[3] / (float)8);
		NVRAM0[(1320 + 15)] = (uint16_t)((float)sum[4] / (float)8);
		NVRAM0[(1320 + 16)] = (uint16_t)((float)sum[5] / (float)8);


#line 58 "..\\..\\sPlcPort\\sPlcAdc.c"
