...
#line 2 "..\\..\\sPlcPort\\sPlcIoIn.c"
 
int8_t inputFilter[(1311 - 1304 + 1) * 16];
 
void sPlcInputInit(void){
	memset((uint8_t*)inputFilter, 0x0, ((1311 - 1304 + 1) * 16));
	printf("%s,%d,%s:input init......\n","..\\..\\sPlcPort\\sPlcIoIn.c", 7, __func__);
}
void sPlcInputRefresh(void){
	uint8_t	temp;
	
	temp = HAL_GPIO_ReadPin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0400UL)), ((uint16_t)0x4000));
	if(temp == 0){
		if(inputFilter[0] < 8){
			inputFilter[0] ++;
		}
		else{
			NVRAM0[1304] |= (int16_t)(1 << 0);
		}
	}
	else{
		if(inputFilter[0] > (8 * -1)){
			inputFilter[0] --;
		}
		else{
			NVRAM0[1304] &= ~(uint16_t)(1 << 0);
		}
	}
	
	temp = HAL_GPIO_ReadPin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0400UL)), ((uint16_t)0x0002));
	if(temp == 0){
		if(inputFilter[1] < 8){
			inputFilter[1] ++;
		}
		else{
			NVRAM0[1304] |= (int16_t)(1 << 1);
		}
	}
	else{
		if(inputFilter[1] > (8 * -1)){
			inputFilter[1] --;
		}
		else{
			NVRAM0[1304] &= ~(uint16_t)(1 << 1);
		}
	}

	
	temp = HAL_GPIO_ReadPin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0400UL)), ((uint16_t)0x0001));
	if(temp == 0){
		if(inputFilter[2] < 8){
			inputFilter[2] ++;
		}
		else{
			NVRAM0[1304] |= (int16_t)(1 << 2);
		}
	}
	else{
		if(inputFilter[2] > (8 * -1)){
			inputFilter[2] --;
		}
		else{
			NVRAM0[1304] &= ~(uint16_t)(1 << 2);
		}
	}
	
	temp = HAL_GPIO_ReadPin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)), ((uint16_t)0x0020));
#line 87 "..\\..\\sPlcPort\\sPlcIoIn.c"
	if(temp == 1){
		if(inputFilter[3] < 8){
			inputFilter[3] ++;
		}
		else{
			NVRAM0[1304] |= (int16_t)(1 << 3);
		}
	}
	else{
		if(inputFilter[3] > (8 * -1)){
			inputFilter[3] --;
		}
		else{
			NVRAM0[1304] &= ~(uint16_t)(1 << 3);
		}
	}

	
	if(NVRAM0[(1320 + 13)] <= deviceConfig.fiberDetect){
		if(inputFilter[4] < 8){
			inputFilter[4] ++;
		}
		else{
