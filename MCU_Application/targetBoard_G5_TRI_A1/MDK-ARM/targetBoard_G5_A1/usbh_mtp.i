#line 1 "../Middlewares/ST/STM32_USB_Host_Library/Class/MTP/Src/usbh_mtp.c"
...
#line 44 "../Middlewares/ST/STM32_USB_Host_Library/Class/MTP/Src/usbh_mtp.c"
...
static USBH_StatusTypeDef USBH_MTP_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  interface = USBH_FindInterface(phost, 0x06U, 1U, 1U);
  if ((interface == 0xFFU) || (interface >= 10U))  
  {
    do { printf("DEBUG : "); printf("Cannot Find the interface for Still Image Class."); printf("\n"); } while (0);
    return USBH_FAIL;
  }
  ...
  if ((endpoint == 0xFFU) || (endpoint >= 5U))
  {
    do { printf("DEBUG : "); printf("Invalid Control endpoint number"); printf("\n"); } while (0);
    return USBH_FAIL;
  }

  phost->pActiveClass->pData = (MTP_HandleTypeDef *)malloc(sizeof(MTP_HandleTypeDef));
  MTP_Handle = (MTP_HandleTypeDef *)phost->pActiveClass->pData;

  if (MTP_Handle == 0)
  {
    do { printf("DEBUG : "); printf("Cannot allocate memory for MTP Handle"); printf("\n"); } while (0);
    return USBH_FAIL;
  }

   
  (void)memset(MTP_Handle, 0, sizeof(MTP_HandleTypeDef));
  ...
  if ((endpoint == 0xFFU) || (endpoint >= 5U))
  {
    do { printf("DEBUG : "); printf("Invalid Data IN endpoint number"); printf("\n"); } while (0);
    return USBH_FAIL;
  }
  ...
  if ((endpoint == 0xFFU) || (endpoint >= 5U))
  {
    do { printf("DEBUG : "); printf("Invalid Data OUT endpoint number"); printf("\n"); } while (0);
    return USBH_FAIL;
  }
...
static uint8_t MTP_FindCtlEndpoint(USBH_HandleTypeDef *phost)
{
  ...
  for (interface = 0U; interface < 10U; interface ++)
  {
    if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass == 0x06U)
    {
      for (endpoint = 0U; endpoint < 5U; endpoint ++)
      {
        if (((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bEndpointAddress & 0x80U) != 0U) &&
            ...
            ((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bmAttributes & 3U) == 3U))
        {
          return endpoint;
        }
      }
  ...
  for (interface = 0U; interface < 10U; interface ++)
  {
    if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass == 0x06U)
    {
      for (endpoint = 0U; endpoint < 5U; endpoint ++)
      {
        ...
        if (((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bEndpointAddress & 0x80U) == 0U) &&
            (phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].wMaxPacketSize > 0U) &&
            ((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bmAttributes & 2U) == 2U))
        {
          return endpoint;
        }
      }
...
static USBH_StatusTypeDef USBH_MTP_InterfaceDeInit(USBH_HandleTypeDef *phost)
{
  ...
  if (MTP_Handle->NotificationPipe != 0U)
  {
    ...
    MTP_Handle->NotificationPipe = 0U;      
  }
...
static USBH_StatusTypeDef USBH_MTP_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (MTP_Handle->state)
  {
    case  MTP_OPENSESSION:
      status = USBH_PTP_OpenSession(phost, 1U);   

      if (status == USBH_OK)
      {
        do { printf("MTP Session #0 Opened"); printf("\n"); } while (0);
    ...
    case MTP_GETDEVICEINFO:
      ...
      if (status == USBH_OK)
      {
        do { printf("DEBUG : "); printf(">>>>> MTP Device Information"); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Standard version : %x", MTP_Handle ->info . devinfo . StandardVersion); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Vendor ExtID : %s", (MTP_Handle ->info . devinfo . VendorExtensionID == 6) ? "MTP" : "NOT SUPPORTED"); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Functional mode : %s", (MTP_Handle ->info . devinfo . FunctionalMode == 0U) ? "Standard" : "Vendor"); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Number of Supported Operation(s) : %d", MTP_Handle ->info . devinfo . OperationsSupported_len); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Number of Supported Events(s) : %d", MTP_Handle ->info . devinfo . EventsSupported_len); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Number of Supported Proprieties : %d", MTP_Handle ->info . devinfo . DevicePropertiesSupported_len); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Manufacturer : %s", MTP_Handle ->info . devinfo . Manufacturer); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Model : %s", MTP_Handle ->info . devinfo . Model); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Device version : %s", MTP_Handle ->info . devinfo . DeviceVersion); printf("\n"); } while (0);
        do { printf("DEBUG : "); printf("Serial number : %s", MTP_Handle ->info . devinfo . SerialNumber); printf("\n"); } while (0);
    ...
    case MTP_GETSTORAGEIDS:
      ...
      if (status == USBH_OK)
      {
        do { printf("DEBUG : "); printf("Number of storage ID items : %d", MTP_Handle ->info . storids . n); printf("\n"); } while (0);
        for (idx = 0U; idx < MTP_Handle->info.storids.n; idx ++)
        {
          do { printf("DEBUG : "); printf("storage#%d ID : %x", idx, MTP_Handle ->info . storids . Storage[idx]); printf("\n"); } while (0);
        }
    ...
    case MTP_GETSTORAGEINFO:
      ...
      if (status == USBH_OK)
      {
        do { printf("Volume#%lu: %s   [%s]", MTP_Handle ->current_storage_unit, MTP_Handle ->info . storinfo[MTP_Handle ->current_storage_unit]. StorageDescription, MTP_Handle ->info . storinfo[MTP_Handle ->current_storage_unit]. VolumeLabel); printf("\n"); } while (0);
        ...
        if (++MTP_Handle->current_storage_unit >= MTP_Handle->info.storids.n)
        {
          ...
          do { printf("MTP Class initialized."); printf("\n"); } while (0);
          do { printf("%s is default storage unit", MTP_Handle ->info . storinfo[0]. StorageDescription); printf("\n"); } while (0);
...
USBH_StatusTypeDef USBH_MTP_GetStorageInfo(USBH_HandleTypeDef *phost, uint8_t storage_idx, PTP_StorageInfoTypedef *info)
{
  USBH_StatusTypeDef status = USBH_FAIL;
  MTP_HandleTypeDef *MTP_Handle = (MTP_HandleTypeDef *)phost->pActiveClass->pData;
...
static void MTP_DecodeEvent(USBH_HandleTypeDef *phost)
{
  ...
  {
    case 0x4000U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_Undefined in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4001U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_CancelTransaction in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4002U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_ObjectAdded in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4003U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_ObjectRemoved in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4004U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_StoreAdded in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4005U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_StoreRemoved in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4006U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_DevicePropChanged in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4007U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_ObjectInfoChanged in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4008U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_DeviceInfoChanged in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x4009U:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_RequestObjectTransfer in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x400AU:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_StoreFull in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x400BU:
      do { printf("DEBUG : "); printf("EVT: PTP_EC_DeviceReset in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x400CU :
      do { printf("DEBUG : "); printf("EVT: PTP_EC_StorageInfoChanged in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x400DU :
      do { printf("DEBUG : "); printf("EVT: PTP_EC_CaptureComplete in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    case 0x400EU :
      do { printf("DEBUG : "); printf("EVT: PTP_EC_UnreportedStatus in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;

    default :
      do { printf("DEBUG : "); printf("Received unknown event in session %u", MTP_Handle ->ptp . session_id); printf("\n"); } while (0);
      break;
  }
