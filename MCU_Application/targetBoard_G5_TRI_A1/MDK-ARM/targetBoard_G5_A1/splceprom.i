...
HAL_StatusTypeDef epromReadByte(uint16_t ReadAddr, uint8_t *rdat){
...
HAL_StatusTypeDef epromReadHword(uint16_t ReadAddr, uint16_t *rdat){



	HAL_StatusTypeDef ret;
	if((ReadAddr + 1) > (8192 - 1)){
		ret = HAL_ERROR;
		return ret;
	}	
	ret = HAL_I2C_Mem_Read(&hi2c1, 
...
HAL_StatusTypeDef epromReadDword(uint16_t ReadAddr, uint32_t *rdat){



	HAL_StatusTypeDef ret;	
	if((ReadAddr + 3) > (8192 - 1)){
		ret = HAL_ERROR;
		return ret;
	}	
	ret = HAL_I2C_Mem_Read(&hi2c1, 
...
HAL_StatusTypeDef epromWriteByte(uint16_t WriteAddr, uint8_t *wdat){
...
HAL_StatusTypeDef epromWriteHword(uint16_t WriteAddr, uint16_t *wdat){



	HAL_StatusTypeDef ret;
	if((WriteAddr + 1) > (8192 - 1)){
		ret = HAL_ERROR;
		return ret;
	}
	ret = HAL_I2C_Mem_Write(&hi2c1, 
...
HAL_StatusTypeDef epromWriteDword(uint16_t WriteAddr, uint32_t *wdat){



	HAL_StatusTypeDef ret;
	if((WriteAddr + 3) >= (8192 - 1)){
		ret = HAL_ERROR;
		return ret;
	}
	ret = HAL_I2C_Mem_Write(&hi2c1, 
...
HAL_StatusTypeDef epromRead(uint16_t ReadAddr, uint8_t *pBuffer, uint16_t NumToRead){
	...
	HAL_StatusTypeDef ret;
	volatile uint16_t rAddr, rBlock, rByte, doBlock;
	uint8_t* rBuffer;
	if((ReadAddr + NumToRead) > 8192){
		ret = HAL_ERROR;
		return ret;
	}
...
void listEpromTable(void){
	printf("MR EPROM:0x%04X---0x%04X(size:%d)\n", (uint32_t)(0x0L), (uint32_t)((0x0L) + 31 - 0), (uint16_t)(31 - 0 + 1));
	printf("DM EPROM:0x%04X---0x%04X(size:%d)\n", (uint32_t)(((0x0L) + 31 - 0) + 1), (uint32_t)((((0x0L) + 31 - 0) + 1) + 543 - 32), (uint16_t)(543 - 32 + 1));
	printf("FD EPROM:0x%04X---0x%04X(size:%d)\n", (uint32_t)(((((0x0L) + 31 - 0) + 1) + 543 - 32) + 1), (uint32_t)((((((0x0L) + 31 - 0) + 1) + 543 - 32) + 1) + 2047 - 0), (uint16_t)(2047 - 0 + 1));
	
	printf("MR CRC EPROM:0x%04X---0x%04X\n", (uint32_t)(7524L), (uint32_t)((7524L) + 3));
	printf("DM CRC EPROM:0x%04X---0x%04X\n", (uint32_t)(7528L), (uint32_t)((7528L) + 3));
	printf("FD CRC EPROM:0x%04X---0x%04X,(unuse)\n", (uint32_t)(7532L), (uint32_t)((7532L) + 3));	
	printf("MCU CRC EPROM:0x%04X---0x%04X\n", (uint32_t)(7536L), (uint32_t)((7536L) + 3));
	printf("LCD CRC EPROM:0x%04X---0x%04X\n", (uint32_t)(7540L), (uint32_t)((7540L) + 3));
	
	printf("CONFIG EPROM:0x%04X---0x%04X(size:%d)\n", (uint32_t)(7552L), (uint32_t)(8063L), (uint16_t)((8063L) - (7552L) + 1));
	printf("LOGINFO EPROM:0x%04X---0x%04X(size:%d)\n", (uint32_t)(8064L),(uint32_t)(8191L), (uint16_t)((8191L) - (8064L) + 1));
}
void clearEprom(clarmEpromCmd_t cmd){
	uint8_t var = 0;
	uint32_t i;	
	switch(cmd){
		...
		case CLEAR_EPROM_NVRAM:{
			for(i = (0x0L); i <= ((0x0L) + 31 - 0);i ++){
				epromWriteByte(i, &var);
			}
			for(i = (((0x0L) + 31 - 0) + 1); i <= ((((0x0L) + 31 - 0) + 1) + 543 - 32);i ++){
				epromWriteByte(i, &var);
			}
			
			for(i = (7524L); i <= ((7524L) + 3);i ++){
				epromWriteByte(i, &var);
			}
			
			for(i = (7528L); i <= ((7528L) + 3);i ++){
				epromWriteByte(i, &var);
			}
			break;
		}
		case CLEAR_EPROM_FDRAM:{
			for(i = (((((0x0L) + 31 - 0) + 1) + 543 - 32) + 1); i <= ((((((0x0L) + 31 - 0) + 1) + 543 - 32) + 1) + 2047 - 0);i ++){
				epromWriteByte(i, &var);
			}			
			for(i = (7532L); i <= ((7532L) + 3);i ++){
				epromWriteByte(i, &var);
			}
			break;
		}
		case CLEAR_EPROM_FIRMWARE_CRC:{
			for(i = (7536L);i <= ((7536L) + 3);i ++){
				epromWriteByte(i, &var);
			}
			for(i = (7540L);i <= ((7540L) + 3);i ++){
				epromWriteByte(i, &var);
			}
			break;
		}
...
uint8_t checkBlank(uint32_t adr, uint32_t size){
	uint8_t val;
	uint32_t i;
	for(i = 0;i < size;i ++){
...
uint8_t sPlcEpromTest(void){
	...
	if(crcRead == crcWrite){
		printf("%s,%d,%s:dword(32bit) sequential write pass!\r\n", "..\\..\\sPlcPort\\sPlcEprom.c", 378, __func__);
	}
	else{
		printf("%s,%d,%s:dword(32bit) sequential wirte fail!\r\n", "..\\..\\sPlcPort\\sPlcEprom.c", 381, __func__);
		res = 0;
	}
	
	bk = 8192 / sizeof(wblock);
	remain =  8192 % sizeof(wblock);
	for(i = 0; i < bk;i ++){
		...
		if(cmpByte(rblock, rblock, sizeof(rblock)) == 0){
			printf("%s,%d,%s:block sequential Write fail!\r\n", "..\\..\\sPlcPort\\sPlcEprom.c", 393, __func__);
			res = 0;
		}
	}
	memset(wblock, (rand() % 0xFF), sizeof(wblock));
	memset(rblock, 0x00, sizeof(rblock));
	epromWrite((8192 - remain), wblock, remain);
	epromRead((8192 - remain), rblock, remain);
	if(cmpByte(rblock, rblock, sizeof(rblock)) == 0){
		printf("%s,%d,%s:block sequential Write fail!\r\n", "..\\..\\sPlcPort\\sPlcEprom.c", 402, __func__);
		res = 0;
	}
	printf("%s,%d,%s:block sequential Write pass!\r\n", "..\\..\\sPlcPort\\sPlcEprom.c", 405, __func__);
	
	for(i=0;i<10000;i++){
		tempWrite = HAL_RNG_GetRandomNumber(&hrng)%(8192- 5 -0+1 ) + 0;;
		epromWrite(bk, ((uint8_t*)&tempWrite), 4);
		epromRead(bk, ((uint8_t*)&tempRead), 4);
		if(tempWrite != tempRead){
			printf("%s,%d,%s:random write fail!\r\n", "..\\..\\sPlcPort\\sPlcEprom.c", 412, __func__);
			res = 0;
		}
	}
	printf("%s,%d,%s:random write pass!\r\n", "..\\..\\sPlcPort\\sPlcEprom.c", 416, __func__);
	return res;
}
