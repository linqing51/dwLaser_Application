...
#line 2 "..\\..\\sPlcPort\\sPlcSpeaker.c"
 
static int8_t LoudspeakerEnable = -1;
static int8_t LoudspeakerVolume = -1;
 
void sPlcSpeakerInit(void){
	RRES((1384 * 16 + 32));
	sPlcSpeakerVolume(NVRAM0[(32 + 2)]);
	sPlcSpeakerDisable();
	sPlcSpeakerFreq(1100);
	printf("%s,%d,%s:speaker dma init......\n","..\\..\\sPlcPort\\sPlcSpeaker.c", 11, __func__);
}

void sPlcSpeakerDisable(void){
	if(LoudspeakerEnable != 0){
...
void sPlcSpeakerEnable(void){
	if(LoudspeakerEnable != 1){
...
void sPlcSpeakerFreq(int16_t freq){
	float32_t f1;
	HAL_TIM_Base_Stop(&htim7);HAL_DAC_Stop_DMA(&hdac, 0x00000000U);;
	if(freq > 4500L){
		freq = 4500L;
	}
	if(freq < 500){
	...
	htim7.Init.ClockDivision = 0x00000000U;
	htim7.Init.AutoReloadPreload = (0x1UL << (7U));
	if (HAL_TIM_Base_Init(&htim7) != HAL_OK){
		printf("%s,%d,%s:tim7 init fail!\n","..\\..\\sPlcPort\\sPlcSpeaker.c", 53, __func__);
		Error_Handler();
	}
...
void sPlcSpeakerVolume(int16_t volume){
	int16_t i;
	float64_t fvolume = 0;
