#line 1 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Src/usbh_hid.c"
...
#line 43 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Src/usbh_hid.c"
#line 1 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_parser.h"
...
#line 29 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_parser.h"
#line 1 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
...
#line 68 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
...
#line 87 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
...
#line 116 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
 
#line 125 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
 
#line 146 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
 
#line 156 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
 
#line 165 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_usage.h"
...
#line 30 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Inc/usbh_hid_parser.h"
...
#line 44 "../Middlewares/ST/STM32_USB_Host_Library/Class/HID/Src/usbh_hid.c"
...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, 0x01U, 0xFFU);
  ...
  phost->pActiveClass->pData = (HID_HandleTypeDef *)malloc(sizeof(HID_HandleTypeDef));
  HID_Handle = (HID_HandleTypeDef *) phost->pActiveClass->pData;

  if (HID_Handle == 0)
  {
    do { printf("DEBUG : "); printf("Cannot allocate memory for HID Handle"); printf("\n"); } while (0);
    return USBH_FAIL;
  }

   
  (void)memset(HID_Handle, 0, sizeof(HID_HandleTypeDef));

  HID_Handle->state = USBH_HID_ERROR;

   
  if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol == 0x01U)
  {
    do { printf("KeyBoard device found!"); printf("\n"); } while (0);
    HID_Handle->Init = USBH_HID_KeybdInit;
  }
  else if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol  == 0x02U)
  {
    do { printf("Mouse device found!"); printf("\n"); } while (0);
    HID_Handle->Init = USBH_HID_MouseInit;
  }
  else
  {
    do { printf("Protocol not supported."); printf("\n"); } while (0);
    return USBH_FAIL;
  }
  ...
  max_ep = ((phost->device.CfgDesc.Itf_Desc[interface].bNumEndpoints <= 5U) ?
            phost->device.CfgDesc.Itf_Desc[interface].bNumEndpoints : 5U);
  ...
  for (num = 0U; num < max_ep; num++)
  {
    if ((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[num].bEndpointAddress & 0x80U) != 0U)
    {
      ...
      (void)USBH_OpenPipe(phost, HID_Handle->InPipe, HID_Handle->InEp, phost->device.address,
                          phost->device.speed, 0x03U, ep_mps);
...
extern uint8_t isLegalUsbDev(USBH_HandleTypeDef *phost);
static USBH_StatusTypeDef USBH_HID_ClassRequest(USBH_HandleTypeDef *phost)
{
  ...
  switch (HID_Handle->ctl_state)
  {
    ...
    case USBH_HID_REQ_GET_REPORT_DESC:
      ...
      {
        do { printf("ERROR: "); printf("Control error: HID: Device Get Report Descriptor request failed"); printf("\n"); } while (0);
        status = USBH_FAIL;
      }
      else
      {
      ...
      if (classReqStatus == USBH_OK)
      {
        ...
        status = USBH_OK;
				...
				if(isLegalUsbDev(phost)){
					...
					do { printf("Find KACON HRF-M9G USB HID Device,skip set protocol\n"); printf("\n"); } while (0);
					phost->pUser(phost, 0x02U);
					status = USBH_OK;
				}
				else{
					do { printf("ERROR: "); printf("Control error: HID: Device Set protocol request failed"); printf("\n"); } while (0);
					status = USBH_FAIL;
				}
			}
      else
...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (HID_Handle->state)
  {
    case USBH_HID_INIT:
      ...
      {
        do { printf("ERROR: "); printf("HID Class Init failed"); printf("\n"); } while (0);
        HID_Handle->state = USBH_HID_ERROR;
        status = USBH_FAIL;
      }
    ...
    case USBH_HID_POLL:
      if (USBH_LL_GetURBState(phost, HID_Handle->InPipe) == USBH_URB_DONE)
      {
        ...
        if ((HID_Handle->DataReady == 0U) && (XferSize != 0U) && (HID_Handle->fifo.buf != 0))
        {
                                                   ...
                                                   uint16_t length)
{
  ...
  {
    do { printf("ERROR: "); printf("Control error: Get HID Report Descriptor failed, data buffer size issue"); printf("\n"); } while (0);
                                             ...
                                             uint16_t length)
{
  ...
  if (length > sizeof(phost->device.Data))
  {
    do { printf("ERROR: "); printf("Control error: Get HID Descriptor failed, data buffer size issue"); printf("\n"); } while (0);
    return USBH_NOT_SUPPORTED;
  }
                                    ...
                                    uint8_t reportId)
{
  ...
  phost->Control.setup.b.bmRequestType = 0x00U | 0x01U |                                          0x20U;



  phost->Control.setup.b.bRequest = 0x0AU;
...
static void USBH_HID_ParseHIDDesc(HID_DescTypeDef *desc, uint8_t *buf)
{
  ...
  CfgDescLen = (((uint16_t)(buf + 2U)[0]) | ((uint16_t)(((uint32_t)(buf + 2U)[1]) << 8)));
  ...
  if (CfgDescLen > 0x09U)
  {
    ...
    while (ptr < CfgDescLen)
    {
      ...
      if (pdesc->bDescriptorType == 0x21U)
      {
        desc->bLength = *(uint8_t *)((uint8_t *)pdesc + 0U);
        desc->bDescriptorType = *(uint8_t *)((uint8_t *)pdesc + 1U);
        desc->bcdHID = (((uint16_t)((uint8_t *)pdesc + 2U)[0]) | ((uint16_t)(((uint32_t)((uint8_t *)pdesc + 2U)[1]) << 8)));
        desc->bCountryCode = *(uint8_t *)((uint8_t *)pdesc + 4U);
        desc->bNumDescriptors = *(uint8_t *)((uint8_t *)pdesc + 5U);
        desc->bReportDescriptorType = *(uint8_t *)((uint8_t *)pdesc + 6U);
        desc->wItemLength = (((uint16_t)((uint8_t *)pdesc + 7U)[0]) | ((uint16_t)(((uint32_t)((uint8_t *)pdesc + 7U)[1]) << 8)));
        break;
      }
    }
  }
...
HID_TypeTypeDef USBH_HID_GetDeviceType(USBH_HandleTypeDef *phost)
{
  ...
  if (phost->gState == HOST_CLASS)
  {
    ...
    if (InterfaceProtocol == 0x01U)
    {
      type = HID_KEYBOARD;
    }
    else
    {
      if (InterfaceProtocol == 0x02U)
      {
        type = HID_MOUSE;
      }
    }
