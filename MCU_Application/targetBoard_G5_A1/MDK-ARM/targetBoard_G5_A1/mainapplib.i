...
#line 2 "..\\..\\MainApp\\MainAppLib.c"
 
void addAcousticTime(void){
	NVRAM0[(576 + 78)] += NVRAM0[(576 + 80)];
	LIMS16((576 + 78), (576 + 84), (576 + 83));
	NVRAM0[(576 + 77)] += NVRAM0[(576 + 79)];
	LIMS16((576 + 77), (576 + 82), (576 + 81));
}
void decAcousticTime(void){
	NVRAM0[(576 + 78)] -= NVRAM0[(576 + 80)];
	LIMS16((576 + 78), (576 + 84), (576 + 83));
...
void updateCustomScheme(uint8_t index){
	uint8_t *psrc, *pdist;
	pdist = (uint8_t*)&FDRAM0[index * ((0 + 63) - (0 + 0) + 1)];
	psrc = (uint8_t*)&NVRAM0[(576 + 0)];
	memcpy(pdist, psrc, (((0 + 63) - (0 + 0) + 1) * 2));
}
...
uint16_t fitLaserToCodeLine(uint8_t ch, int16_t power){
	...
	switch(ch){
		case 0x01:{
			tmp = (uint16_t)((int32_t)power * 4095 / 150);
			break;
		}
		case 0x02:{
			tmp = (uint16_t)((int32_t)power * 4095 / 150);
...
uint16_t fitLaserToCode(uint8_t ch, int16_t power, deviceConfig_t *pcfg){
	...
	fpower = (float)power;
	printf("%s,%d,%s:Calibration input power = %d\n", "..\\..\\MainApp\\MainAppLib.c", 147, __func__, power);
	printf("%s,%d,%s:Calibration max power = %d\n", "..\\..\\MainApp\\MainAppLib.c", 148, __func__, pmax);
	printf("%s,%d,%s:Calibration min power = %d\n", "..\\..\\MainApp\\MainAppLib.c", 149, __func__, pmin);
	if(LDB((544 * 16 + 22))){
		...
		fout = (power - fb) / fk;
		printf("%s,%d,%s:Enable calibration mode\n", "..\\..\\MainApp\\MainAppLib.c", 194, __func__);
		printf("%s,%d,%s:Calibration fK=%f, fB=%f\n", "..\\..\\MainApp\\MainAppLib.c", 195, __func__, fk, fb);
		printf("%s,%d,%s:Calibration fOut=%f\n", "..\\..\\MainApp\\MainAppLib.c", 196, __func__, fout);
	}
	else{
		printf("%s,%d,%s:Disable calibration mode\n", "..\\..\\MainApp\\MainAppLib.c", 199, __func__);
		fout = fpower / pmax;			
	}
	fout = fout * 4095.0F;
	if(fout > 4095.0F){
	...
	out = (uint16_t)fout;
	printf("%s,%d,%s:Calibration fOut=%d\n", "..\\..\\MainApp\\MainAppLib.c", 210, __func__, out);
	return out;
}
