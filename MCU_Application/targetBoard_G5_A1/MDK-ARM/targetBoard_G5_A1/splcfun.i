...
void SSET(uint16_t A){

	sPlcAssertCoilAddress(A);

	NVRAM0[(A / 16)] |= 1 << (A % 16);
}
void RRES(uint16_t A){
...
void T10MS(uint8_t A, uint8_t start, uint16_t value){
	if(start){
		SSET(1104 * 16 + A);
		if(NVRAM0[(1176 + A)] >= value){
			NVRAM0[(1092 + (A / 16))] |= 1 << (A % 16);
		}
		else{
			NVRAM0[(1092 + (A / 16))] &= ~(1 << (A % 16));
		}
...
void T100MS(uint8_t A, uint8_t start, uint16_t value){
	if(start){
		SSET(1108 * 16 + A);
		if(NVRAM0[(1240 + A)] >= value){
			NVRAM0[(1096 + (A / 16))] |= 1 << (A % 16);
		}
		else{		
			NVRAM0[(1096 + (A / 16))] &= ~(1 << (A % 16));
		}	
...
void CLR(uint16_t A){
	...
	sPlcAssertRegisterAddress(A);

	NVRAM0[A] = 0x0;
}
void CLRD(uint16_t A){

	sPlcAssertRegisterAddress(A);
	sPlcAssertRegisterAddress(A + 1);

	NVRAM0[A] = 0x0;NVRAM0[A + 1] = 0x0;
}
void MOV(uint16_t dist, uint16_t src){

	sPlcAssertRegisterAddress(dist);
	sPlcAssertRegisterAddress(src);

	NVRAM0[dist] = NVRAM0[src];
}
void MOVD(uint16_t dist, uint16_t src){

	sPlcAssertRegisterAddress(dist);
	sPlcAssertRegisterAddress(dist+1);
	sPlcAssertRegisterAddress(src);
	sPlcAssertRegisterAddress(src+1);
...
void TNTUC(uint16_t dist, uint16_t src){
	float32_t ftemp;
	...
	NVRAM0[(1400 + 0)] = 0;
	NVRAM0[(1400 + 1)] = 0xFFF;
	LIMS16(src, (1400 + 0), (1400 + 1));
	
	if(NVRAM0[src] >= 10){
		if(NVRAM0[(1320 + 25)]  > 0){
			ftemp = (3300.0F * *(volatile uint16_t *)(0x1FFF7A2A) * NVRAM0[src]) / (NVRAM0[(1320 + 25)] * 4096.0F);
		}
		else{
			ftemp = (3300.0F * *(volatile uint16_t *)(0x1FFF7A2A) * NVRAM0[src]) / 4096.0F;
		}
		if(ftemp == 3300L){
			ftemp = 999999.9F;
		}
		else{
			ftemp = ftemp * 4700L / (3300L - ftemp);
		}
	}
	else{
		ftemp = 0.1F;
	}
	ftemp = 1 / (1 / (273.15F + 25.0F) + 1 / 3477.0F * log(ftemp / 10000.0F)) - 273.15;
	if(ftemp >= 100) ftemp = 100;
	if(ftemp <= -100) ftemp = -100;
	NVRAM0[dist] = (int16_t)(ftemp * 10);
}

void TNTLC(uint16_t dist, uint16_t src){
	float32_t ftemp;
...
void TENV(uint16_t dist, uint16_t src){
	float32_t ftemp;
	if(NVRAM0[(1320 + 25)]  > 0){
		ftemp = (3300.0F * *(volatile uint16_t *)(0x1FFF7A2A) * NVRAM0[src]) / (NVRAM0[(1320 + 25)] * 4096.0F);
	}
...
void ADD1(uint16_t Sa){
	NVRAM0[Sa] += 1;
}
void ADDS1(uint16_t Sa){
	if(NVRAM0[Sa] < 0x7fff){
		NVRAM0[Sa] += 1;
	}
}
void DEC1(uint16_t Sa){
	NVRAM0[Sa] -= 1;
}
void DECS1(uint16_t Sa){
	if(NVRAM0[Sa] > (-0x8000)){
		NVRAM0[Sa] -= 1;
	}
}
void ADL1(uint16_t Sa){
	int32_t *p;
	p = (int32_t*)(&NVRAM0[Sa]);	
	(*p) ++;
}
void ADLS1(uint16_t Sa){
	int32_t *p;
	p = (int32_t*)(&NVRAM0[Sa]);
	if(*p < 0x7fffffffL){
		(*p) ++;
...
void DEL1(uint16_t Sa){
	int32_t *p;
	p = (int32_t*)(&NVRAM0[Sa]);
	(*p) --;
}
void DELS1(uint16_t Sa){
	int32_t *p;
	p = (int32_t*)(&NVRAM0[Sa]);
	if(*p > (~0x7fffffffL)){
		(*p) --;
...
void ADD16(uint16_t Sa, uint16_t Sb, uint16_t D){
	NVRAM0[D] = NVRAM0[Sa] + NVRAM0[Sb];
}
void ADDS16(uint16_t Sa, uint16_t Sb, uint16_t D){
	int32_t tmp;
...
void ADD32(uint16_t Sa, uint16_t Sb, uint16_t D){
...
void ADDS32(uint16_t Sa, uint16_t Sb, uint16_t D){
	int32_t tmpSa, tmpSb;
	int64_t tmpD;
	tmpSa = *((int32_t*)(&NVRAM0[Sa]));
	tmpSb = *((int32_t*)(&NVRAM0[Sb]));
...
void ADDS32D(uint16_t Sa, uint16_t Sb, uint16_t D){
	int32_t tmpSa, tmpSb;
	int64_t tmpD;
	tmpSa = *((int32_t*)(&NVRAM0[Sa]));
	tmpSb = *((int32_t*)(&NVRAM0[Sb]));
...
void SUB16(uint16_t Sa, uint16_t Sb, uint16_t D){
	NVRAM0[D] = NVRAM0[Sa] - NVRAM0[Sb];
}
void SUBS16(uint16_t Sa, uint16_t Sb, uint16_t D){
	int32_t tmp;
	tmp = NVRAM0[Sa] - NVRAM0[Sb];
	if(tmp >= 0x7fff)
		tmp = 0x7fff;
...
void SUB32(uint16_t Sa, uint16_t Sb, uint16_t D){
	int32_t tmpSa, tmpSb;
	int64_t tmpD;
	tmpSa = *((int32_t*)(&NVRAM0[Sa]));
	tmpSb = *((int32_t*)(&NVRAM0[Sb]));
...
void MULT16(uint16_t Sa, uint16_t Sb, uint16_t D){
	int32_t tmp = (int32_t)NVRAM0[Sa] * (int32_t)NVRAM0[Sb];
	NVRAM0[D] = (int16_t)(tmp & 0xFFFF);
}
void MULTS16(uint16_t Sa, uint16_t Sb, uint16_t D){
	int32_t tmp = (int32_t)NVRAM0[Sa] * (int32_t)NVRAM0[Sb];
	if(tmp >= 0x7fff)
		tmp = 0x7fff;
	if(tmp <= (-0x8000))
...
void MULT32(uint16_t Sa, uint16_t Sb, uint16_t D){
...
void SUM16(uint16_t index, uint16_t length, uint16_t sum){
	int32_t tmp = 0;
	uint16_t i = 0;
...
void UMAX16(uint16_t index, uint16_t length, uint16_t max){
	uint16_t i = 0;
	uint16_t tmp = 0;
	for(i = 0;i < NVRAM0[length];i ++ ){
...
void UMIN16(uint16_t index, uint16_t length, uint16_t min){
	uint16_t i = 0;
	uint16_t tmp = 0;
	for(i = 0;i < NVRAM0[length];i ++){
		if(tmp < NVRAM0[index + i]){
...
void SMAX16(uint16_t index, uint16_t length, uint16_t max){
	uint16_t i = 0;
	int16_t tmp = 0;
	for(i = 0;i < NVRAM0[length];i ++ ){
		if(tmp < NVRAM0[index + i]){
...
void SMIN16(uint16_t index, uint16_t length, uint16_t min){
	uint16_t i = 0;
	int16_t tmp = 0;
	for(i = 0;i < NVRAM0[length];i ++){
		if(tmp < NVRAM0[index + i]){
...
void ABS16(uint16_t dist, uint16_t src){
	if(NVRAM0[src] >= 0){
		NVRAM0[dist] = NVRAM0[src];
	}
	else{
...
void ABS32(uint16_t dist, uint16_t src){
	if(NVRAM0[src] >= 0){
		NVRAM0[dist] = NVRAM0[src];
	}
	else{
...
void SWAP(uint16_t dist, uint16_t src){
...
void LIMS16(uint16_t src, uint16_t min, uint16_t max){
...
void BCPY(uint16_t dist, uint16_t src, uint16_t length) {
	uint16_t i;
	for(i = 0;i < length;i ++){
...
void FDSAV_ONE(int16_t cn){
	...
	epromWrite((cn * 128 + (((((0x0L) + 31 - 0) + 1) + 543 - 32) + 1)), (uint8_t*)(cn * 64 + FDRAM0), 128);
	printf("%s,%d,%s:save One FD NVRAM done...\n","..\\..\\sPlc\\sPlcFun.c", 493, __func__);
	sPlcIsrEnable();
}
void FDLAD(void){
	sPlcIsrDisable();
