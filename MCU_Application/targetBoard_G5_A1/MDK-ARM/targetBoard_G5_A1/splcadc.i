...
#line 2 "..\\..\\sPlcPort\\sPlcAdc.c"
 
volatile uint16_t adcDmaBuffer0[(7 * 8)];
volatile uint16_t adcDmaBuffer1[(7 * 8)];
volatile uint8_t adcBufferSelect;
volatile uint8_t adcBufferDone;
 
void sPlcAdcInit(void){
	adcBufferSelect = 0;
	memset((uint8_t*)adcDmaBuffer0, 0x0, ((7 * 8) * 2));
	memset((uint8_t*)adcDmaBuffer1, 0x0, ((7 * 8) * 2));
	HAL_ADC_Start_DMA(&hadc1, (uint32_t*)&adcDmaBuffer0, (7 * 8)); 
	printf("%s,%d,%s:start adc init......\n","..\\..\\sPlcPort\\sPlcAdc.c", 13, __func__);
}
...
void sPlcAdcProcess(void){
	...
	if(adcBufferSelect == 0){	
		...
		HAL_ADC_Start_DMA(&hadc1, (uint32_t*)&adcDmaBuffer1, (7 * 8));
		adcBufferSelect = 1;
		memset((uint8_t*)sum, 0x0, (7 * 4));
		for(i = 0;i < 7;i ++){
			for(j = 0;j < 8 ;j ++){
				sum[i] += adcDmaBuffer0[(j * 7 + i)];
			
			}
		}			

		NVRAM0[(1320 + 23)] = (uint16_t)((float)sum[0] / (float)8);
		NVRAM0[(1320 + 22)] = (uint16_t)((float)sum[1] / (float)8);
		NVRAM0[(1320 + 21)] = (uint16_t)((float)sum[2] / (float)8);
		NVRAM0[(1320 + 20)] = (uint16_t)((float)sum[3] / (float)8);
		NVRAM0[(1320 + 24)] = (uint16_t)((float)sum[4] / (float)8);
		NVRAM0[(1320 + 25)] = (uint16_t)((float)sum[5] / (float)8);
		NVRAM0[(1320 + 26)] = (uint16_t)((float)sum[6] / (float)8);


#line 59 "..\\..\\sPlcPort\\sPlcAdc.c"
