...
#line 2 "..\\..\\sPlcPort\\sPlcMisc.c"
#line 3 "..\\..\\sPlcPort\\sPlcMisc.c"
...
void FanTimerInit(void) {
  ...
  HAL_TIM_PWM_ConfigChannel(&htim3, &sConfigOC, 0x00000000U);
}
...
void setFanSpeed(int16_t speed){
	if(FanSpeed != speed){
		if(speed > 100){
			speed = 100;
		}
		...
		(((0x00000004U) == 0x00000000U) ? ((&htim12)->Instance ->CCR1 = (speed)) : ((0x00000004U) == 0x00000004U) ? ((&htim12)->Instance ->CCR2 = (speed)) : ((0x00000004U) == 0x00000008U) ? ((&htim12)->Instance ->CCR3 = (speed)) : ((&htim12)->Instance ->CCR4 = (speed)));
		if(speed != 0){
			HAL_TIM_PWM_Start(&htim12, 0x00000004U);
		}
		else{
			HAL_TIM_PWM_Stop(&htim12, 0x00000004U);
		}
#line 176 "..\\..\\sPlcPort\\sPlcMisc.c"
		FanSpeed = speed;
		printf("%s,%d,%s:set fan:%d\n","..\\..\\sPlcPort\\sPlcMisc.c", 177, __func__, speed);	
	}
}
...
void morseCodeDiag(uint8_t diag){
	...
	switch(diag){
		case '0':{
			...
			HAL_TIM_PWM_Start(&htim3, 0x00000004U);vTaskDelay(300);HAL_TIM_PWM_Stop(&htim3, 0x00000004U);vTaskDelay(3000);
			
			HAL_TIM_PWM_Start(&htim3, 0x00000004U);vTaskDelay(900);HAL_TIM_PWM_Stop(&htim3, 0x00000004U);vTaskDelay(3000);
			
			HAL_TIM_PWM_Start(&htim3, 0x00000004U);vTaskDelay(900);HAL_TIM_PWM_Stop(&htim3, 0x00000004U);vTaskDelay(3000);
			
			HAL_TIM_PWM_Start(&htim3, 0x00000004U);vTaskDelay(900);HAL_TIM_PWM_Stop(&htim3, 0x00000004U);vTaskDelay(3000);
			
			HAL_TIM_PWM_Start(&htim3, 0x00000004U);vTaskDelay(900);HAL_TIM_PWM_Stop(&htim3, 0x00000004U);
			break;
		}
		case '1':{
		...
		case '5':{
			...
			HAL_TIM_PWM_Start(&htim3, 0x00000004U);vTaskDelay(300);HAL_TIM_PWM_Stop(&htim3, 0x00000004U);
			break;
		}
		case '6':{
...
uint32_t getOriginBootloadCrc(void){
	uint8_t val;
	uint32_t i;
	uint32_t crc32;
	crc32Clear();
	for(i = ((uint32_t)0x08000000);i < (((uint32_t)0x08010000) - 1);i ++){
		val = *(volatile uint8_t*)(i);
		crc32 = crc32CalculateAdd(val);
	}
	return crc32;	
}
uint32_t getOriginAppCrc(void){
	uint8_t val;
	uint32_t i;
	uint32_t crc32;
	crc32Clear();
	for(i = ((uint32_t)0x08010000);i < (((uint32_t)0x080E0000) + 131072 - 1);i ++){
		val = *(volatile uint8_t*)(i);
		crc32 = crc32CalculateAdd(val);
	}
	return crc32;	
