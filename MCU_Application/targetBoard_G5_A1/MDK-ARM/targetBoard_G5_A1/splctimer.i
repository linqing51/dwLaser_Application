...
#line 2 "..\\..\\sPlcPort\\sPlcTimer.c"
 
uint32_t sPlcTick;
 
void sPlcTimerInit(void){
	...
	sPlcTick = 0;
	printf("%s,%d,%s:splc timer init......\n","..\\..\\sPlcPort\\sPlcTimer.c", 19, __func__);
}
void sPlcTimerDisable(void) {
	HAL_TIM_Base_Stop_IT(&htim14);
}
void sPlcTimerEnable(void) {
	HAL_TIM_Base_Start_IT(&htim14);
}
void sPlcTimerIsr(void){
	uint16_t i;
	for(i = 1112;i <= 1175;i ++){
		if(LD(1100 * 16 + (i - 1112))){
			if(NVRAM0[i] < 0x7fff){
				NVRAM0[i] ++;
			}
		}
	...
	if(TimerCounter_100mS > 100){
		for(i = 1240;i < 1303;i ++){
			if(LD(1108 * 16 + (i - 1240))){
				if(NVRAM0[i] < 0x7fff){
					NVRAM0[i] ++;
				}
			}
	...
	if(TimerCounter_200mS > 200){
		TD_200MS_SP = 1;
		TimerCounter_200mS = 0;
	}
	if(TimerCounter_500mS > 500){
		TD_500MS_SP = 1;
		TimerCounter_500mS = 0;
	}
