...
#line 28 "../Middlewares/ST/STM32_USB_Host_Library/Class/MSC/Src/usbh_msc_scsi.c"
                                               ...
                                               uint8_t lun)
{
  ...
  switch (MSC_Handle->hbot.cmd_state)
  {
    case BOT_CMD_SEND:
      ...
      (void)memset(MSC_Handle->hbot.cbw.field.CB, 0, 16U);
                                         ...
                                         SCSI_StdInquiryDataTypeDef *inquiry)
{
  ...
  switch (MSC_Handle->hbot.cmd_state)
  {
    case BOT_CMD_SEND:
      ...
      (void)memset(MSC_Handle->hbot.cbw.field.CB, 0, 10U);
    ...
    case BOT_CMD_WAIT:
      ...
      if (error == USBH_OK)
      {
        (void)memset(inquiry, 0, sizeof(SCSI_StdInquiryDataTypeDef));
        ...
        (void)memcpy(inquiry->vendor_id, &MSC_Handle->hbot.pbuf[8], 8U);
        (void)memcpy(inquiry->product_id, &MSC_Handle->hbot.pbuf[16], 16U);
        (void)memcpy(inquiry->revision_id, &MSC_Handle->hbot.pbuf[32], 4U);
      }
      break;
  ...
  switch (MSC_Handle->hbot.cmd_state)
  {
    ...
    case BOT_CMD_WAIT:
      ...
      if (error == USBH_OK)
      {
        ...
        if (sense_data->asc == 0x3A)
        {
          do { printf("MSC Device MEDIUM not present"); printf("\n"); } while (0);
